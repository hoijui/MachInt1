...
The corresponding decision boundaries are for $C_1$ colored in green and for $C_2$ colored in magenta.

TODO insert pictures

At the set with $k = 1$, it becomes possible that single far spread points creat an own bubble.
It is like overfitting.
The greater the chosen $k$, the higher is the probability, that these errors are eliminated.
...
Good values for $k$ are multiples of 4. we tried with 4, 8, 16 and 32, which all lead to practically identical results.
$\sigma$ has to be at least 0.015, or else it will not be able to detect two classes at all.
